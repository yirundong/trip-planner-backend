...
@router.get("/geocode", summary="获取坐标(地理编码)")
async def get_coordinate(
        address: str = Query(..., description="要查询的详细地址", example="故宫博物院"),
        city: Optional[str] = Query(None, description="城市名称", example="北京")
):
    # 1. 拿到你刚才辛辛苦苦写好的服务对象
    amap_service = get_amap_service()

    # 2. 调用服务里的 geocode 方法（这里就会触发探照灯 print）
    location_data = amap_service.geocode(address=address, city=city)

    # 3. 包装成标准格式返回给 Swagger 网页
    if location_data:
        return {
            "success": True,
            "message": "坐标查询成功",
